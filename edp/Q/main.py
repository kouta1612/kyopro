class segtree:
    ...
    def query(self, l: int, r: int, i: int, ql: int, qr: int) -> int:
        if r <= ql or qr <= l: return 0
        if ql <= l and r <= qr: return self.nodes[i]
        mid = (l+r) // 2
        vl, vr = self.query(l, mid, 2*i+1, ql, qr), self.query(mid, r, 2*i+2, ql, qr)
        return max(vl, vr)
...
seg = segtree(MAX_H+1)
for i in range(N):
    best = seg.query(0, seg.n, 0, 0, h[i])
    seg.update(h[i], best + a[i])

print(seg.query(0, seg.n, 0, 0, seg.n))