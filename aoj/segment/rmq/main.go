...
func (seg *SegmentTree) update(n, v int) {
	...
	for n != 0 {
		n = (n - 1) / 2
		seg.nodes[n] = min(seg.nodes[2*n+1], seg.nodes[2*n+2])
	}
}

func (seg *SegmentTree) query(x, y int) int {
	return seg.getmin(x, y, 0, 0, seg.n)
}

func (seg *SegmentTree) getmin(x, y, k, l, r int) int {
	if y <= l || r <= x {
		return INF
	} else if x <= l && r <= y {
		return seg.nodes[k]
		...
		mid := (l + r) / 2
		lv, rv := seg.getmin(x, y, 2*k+1, l, mid), seg.getmin(x, y, 2*k+2, mid, r)
		return min(lv, rv)
	}
}
