...
func main() {
	...
	p, h := make([]int, k), make([]int, k)
	for i := 0; i < k; i++ {
		p[i], h[i] = ni()-1, ni()
	}

	dist, q := make([]int, n), priorityqueue.New(func(a, b pair) int {
		if a.c < b.c {
			return 1
		} else if a.c == b.c {
			return 0
	...
	for q.Size() > 0 {
		...
		for _, nv := range g[now.v] {
			if dist[nv] < dist[now.v]-1 {
				dist[nv] = dist[now.v] - 1
				q.Push(pair{nv, dist[nv]})
			}
		}
	}
