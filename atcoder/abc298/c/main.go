...
func main() {
	...
	hakos := make([]*set.MultiSet[int], n+1)
	cards := make([]*set.Set[int], 200001)
	for i := 0; i < n+1; i++ {
		hakos[i] = set.NewMultiSet(comparator.Comparator[int](comparator.IntComparator))
	}
	for i := 0; i < 200001; i++ {
		cards[i] = set.New(comparator.Comparator[int](comparator.IntComparator))
	}
	for qi := 0; qi < q; qi++ {
		t := ni()
		if t == 1 {
			...
			v := ni()
			ans := make([]int, 0)
			for it := hakos[v].Begin(); it.IsValid(); it.Next() {
				ans = append(ans, it.Value())
			}
			printIntLn(ans)
		} else {
			v := ni()
			ans := make([]int, 0)
			for it := cards[v].Begin(); it.IsValid(); it.Next() {
				ans = append(ans, it.Value())
			}
			printIntLn(ans)
		}
