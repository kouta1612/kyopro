...
func main() {
	...
	to := map[string]p{">": {0, 1}, "<": {0, -1}, "v": {1, 0}, "^": {-1, 0}}
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if g.datas[i][j] == "S" {
				start = p{i, j}
			} else if g.datas[i][j] == "G" {
				goal = p{i, j}
			} else if dir[g.datas[i][j]] {
				pt := to[g.datas[i][j]]
				ni, nj := i, j
				for {
					ni += pt.i
					...
					if dir[g.datas[ni][nj]] || g.datas[ni][nj] == "#" {
						break
					}
					g.datas[ni][nj] = "X"
				}
	...
	dist := g.bfs([]point{{i: start.i, j: start.j}})

	if dist[goal.i][goal.j] == INF {
		fmt.Println(-1)
	} else {
		fmt.Println(dist[goal.i][goal.j])
	}
}
...
func dij() ([]int, []int) {
	// → ↓ ← ↑ ↘ ↙ ↗ ↖
	var di []int = []int{0, 1, 0, -1}
	var dj []int = []int{1, 0, -1, 0}
	return di, dj
}
...
func (g *grid) bfs(starts []point) [][]int {
	...
	for len(q) > 0 {
		...
		for i := 0; i < 4; i++ {
			...
			if g.datas[ni][nj] == "<" || g.datas[ni][nj] == ">" || g.datas[ni][nj] == "v" || g.datas[ni][nj] == "^" || g.datas[ni][nj] == "X" {
				continue
			}
