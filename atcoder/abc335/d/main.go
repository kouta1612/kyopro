...
func main() {
	...
	var dfs func(now point, pre int)
	dfs = func(now point, pre int) {
		pnx, pny, ok := now.x+dx[pre], now.y+dy[pre], true
		if pnx < 0 || pny < 0 || pnx >= n || pny >= n {
			ok = false
		}
		if ok && ans[pnx][pny] != "" {
			ok = false
		...
		if ok {
			ans[pnx][pny] = itoa(atoi(ans[now.x][now.y]) + 1)
			dfs(point{pnx, pny}, pre)
		}
		...
		for i := 0; i < 4; i++ {
			...
			ans[nx][ny] = itoa(atoi(ans[now.x][now.y]) + 1)
			dfs(point{nx, ny}, i)
		}
	}
	dfs(point{0, 0}, 0)
