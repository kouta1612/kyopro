...
func main() {
	...
	// 一番左のRと一番右のLが衝突するかを判定できれば十分
	minR, maxL := make(map[int]int), make(map[int]int)
	for i := 0; i < n; i++ {
		if s[i] == 'L' {
			if mr, exist := minR[y[i]]; exist && x[i] >= mr {
				fmt.Println("Yes")
				return
			}
		} else {
			if ml, exist := maxL[y[i]]; exist && x[i] <= ml {
				fmt.Println("Yes")
				return
			}
		}
		...
		if s[i] == 'L' {
			maxL[y[i]] = max(maxL[y[i]], x[i])
		} else {
			if mr, exist := minR[y[i]]; exist {
				minR[y[i]] = min(mr, x[i])
			} else {
				minR[y[i]] = x[i]
			}
