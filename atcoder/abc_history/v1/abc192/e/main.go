...
func main() {
	...
	for i := 0; i < m; i++ {
		a, b, t, k := ni()-1, ni()-1, ni(), ni()
		es[a] = append(es[a], edge{to: b, cost: t, k: k})
		es[b] = append(es[b], edge{to: a, cost: t, k: k})
	}

	q := priorityqueue.New(func(x, y edge) int {
		if x.cost < y.cost {
			return -1
	...
	dist, kakutei := make([]int, n), make([]bool, n)
	for i := 0; i < n; i++ {
		dist[i] = INF
	}
	dist[x] = 0

	q.Push(edge{to: x, cost: 0})
	for q.Size() > 0 {
		now := q.Pop()
		...
		for _, e := range es[now.to] {
			cost := e.cost + (dist[now.to]+e.k-1)/e.k*e.k
			if kakutei[e.to] {
				continue
			}
			if dist[e.to] <= cost {
