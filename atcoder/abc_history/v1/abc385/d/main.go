...
func main() {
	...
	xs, ys := make(map[int]*set.Set[int]), make(map[int]*set.Set[int])
	for i := 0; i < n; i++ {
		x[i], y[i] = ni2()
		if xs[y[i]] == nil {
			xs[y[i]] = set.New(comparator.Comparator[int](comparator.IntComparator))
		}
		if ys[x[i]] == nil {
			ys[x[i]] = set.New(comparator.Comparator[int](comparator.IntComparator))
		}
		xs[y[i]].Insert(x[i])
		ys[x[i]].Insert(y[i])
	}
	for i := 0; i < m; i++ {
		...
		if sy == ny {
			destroy(xs, ys, sy, sx, nx)
		} else {
			destroy(ys, xs, sx, sy, ny)
		}
		sx, sy = nx, ny
	}
...
func destroy(xs, ys map[int]*set.Set[int], y, l, r int) {
	if l > r {
		l, r = r, l
	}
	if xs[y] == nil || xs[y].Size() == 0 {
		return
	}
	for {
		it := xs[y].LowerBound(l)
		if it.Equal(xs[y].Last().Next()) {
			break
		}
		if it.Value() > r {
			break
