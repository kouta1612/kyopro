...
func dist(ps []point, is []int) float64 {
	n := len(ps)

	res := 0.0
	for i := 0; i < n-1; i++ {
		dx := math.Abs(float64(ps[is[i]].x - ps[is[i+1]].x))
		dy := math.Abs(float64(ps[is[i]].y - ps[is[i+1]].y))

		res += math.Pow(dx*dx+dy*dy, 0.5)
	}
