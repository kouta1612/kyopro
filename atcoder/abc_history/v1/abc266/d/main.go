...
func main() {
	...
	n := ni()
	t, x, a := make([]int, n), make([]int, n), make([]int, n)
	mp := make(map[int]sunuke)
	for i := 0; i < n; i++ {
		t[i], x[i], a[i] = ni3()
		mp[t[i]] = sunuke{x[i], a[i]}
	}
	...
	for i := 0; i < t[n-1]; i++ {
		...
		if v, exist := mp[ni]; !exist || v.pos > ni {
			continue
		}

		dp[ni][mp[ni].pos] += mp[ni].size
	}

	ans := 0
	for i := 0; i < 5; i++ {
		chmax(&ans, dp[t[n-1]][i])
	}
