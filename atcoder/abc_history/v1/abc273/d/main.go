...
func main() {
	...
	rmp, cmp := make(map[int][]int), make(map[int][]int)
	for i := 0; i < n; i++ {
		r[i], c[i] = ni()-1, ni()-1
		rmp[r[i]] = append(rmp[r[i]], c[i])
		cmp[c[i]] = append(cmp[c[i]], r[i])
	}

	for k := range rmp {
		rmp[k] = append(rmp[k], -1, w)
		sort.Ints(rmp[k])
	}
	for k := range cmp {
		cmp[k] = append(cmp[k], -1, h)
		sort.Ints(cmp[k])
	}
	...
	for i := 0; i < q; i++ {
		...
		if t == 'R' {
			if len(rmp[R]) == 0 {
				...
				it := upperBound(rmp[R], C)
				C = min(rmp[R][it]-1, C+d)
			}
		} else if t == 'L' {
			if len(rmp[R]) == 0 {
				C = max(0, C-d)
			} else {
				it := lowerBound(rmp[R], C) - 1
				C = max(rmp[R][it]+1, C-d)
			}
		} else if t == 'D' {
			if len(cmp[C]) == 0 {
				R = min(h-1, R+d)
			} else {
				it := upperBound(cmp[C], R)
				R = min(cmp[C][it]-1, R+d)
			}
		} else {
			if len(cmp[C]) == 0 {
				R = max(0, R-d)
			} else {
				it := lowerBound(cmp[C], R) - 1
				R = max(cmp[C][it]+1, R-d)
			}
		}
		fmt.Printf("%v %v\n", R+1, C+1)
	}
