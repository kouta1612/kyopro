...
func main() {
	...
	g1, g2 := newGraph(0, true, false), newGraph(0, true, false)
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			g1.addEdge(i, j, d[i][j]*a)
			g2.addEdge(i, j, d[i][j]*b+c)
		}
	}

	dist1, dist2 := g1.dijkstra(map[int]int{0: 0}), g2.dijkstra(map[int]int{n - 1: 0})
	ans := INF
	for i := 0; i < n; i++ {
