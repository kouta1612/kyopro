...
func main() {
	...
	sort.Slice(ices, func(i, j int) bool {
		return ices[i].delicious < ices[j].delicious
	})

	ans, best := 0, ices[n-1]
	for i := 0; i < n-1; i++ {
		if best.flavor == ices[i].flavor {
			chmax(&ans, best.delicious+ices[i].delicious/2)
		} else {
			chmax(&ans, best.delicious+ices[i].delicious)
		}
	}
