...
func main() {
	...
	for ci := 1; ci <= n; ci++ {
		...
		for di := 0; di < 3; di++ {
			for dj := 0; dj < 3; dj++ {
				for i := 0; i < len(gs[ci-1]); i++ {
					for j := 0; j < len(gs[ci-1][i]); j++ {
						if di == 1 && dj == 1 {
							s[len(gs[ci-1])*di+i][len(gs[ci-1])*dj+j] = '.'
						} else {
							s[len(gs[ci-1])*di+i][len(gs[ci-1])*dj+j] = gs[ci-1][i][j]
						}
					}
				}
			}
	...
	for i := 0; i < len(gs[n]); i++ {
		fmt.Println(string(gs[n][i]))
	}
}

func pow(p, c int) int {
	res := 1
	for i := 0; i < c; i++ {
		res *= p
	}
