...
func main() {
	...
	g := make([][]string, h)
	for i := 0; i < h; i++ {
		g[i] = strings.Split(strings.Repeat("#", w), "")
	}
	rs, cs := make([]*set.Set[int], h), make([]*set.Set[int], w)
	for i := 0; i < h; i++ {
		rs[i] = set.New(comparator.IntComparator)
	}
	for i := 0; i < w; i++ {
		cs[i] = set.New(comparator.IntComparator)
	}
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			rs[i].Insert(j)
		...
		if g[i][j] == "#" {
		...
		} else {
			{
				...
				if it.IsValid() {
					...
					if rs[i].Last().IsValid() {
						erase(i, rs[i].Last().Value())
					}
				}
			}
			{
				it := cs[j].LowerBound(i)
				if it.IsValid() {
					erase(it.Value(), j)
				}
				it = cs[j].LowerBound(i)
				if it.IsValid() {
					...
					if cs[j].Last().IsValid() {
						erase(cs[j].Last().Value(), j)
					}
				}
			}
		}
