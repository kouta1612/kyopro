...
func main() {
	...
	n, m := ni2()
	g, uf := make([][]int, n), newUnionFind(n)
	for i := 0; i < n; i++ {
		g[i] = make([]int, 0)
	}
	for i := 0; i < m; i++ {
	...
	k := ni()
	ngs := make(map[ngPair]bool)
	for i := 0; i < k; i++ {
		x, y := ni()-1, ni()-1
		ngs[ngPair{uf.root(x), uf.root(y)}] = true
		ngs[ngPair{uf.root(y), uf.root(x)}] = true
	}

	q := ni()
	for qi := 0; qi < q; qi++ {
		p, q := uf.root(ni()-1), uf.root(ni()-1)
		if ngs[ngPair{p, q}] {
			fmt.Println("No")
		} else {
			fmt.Println("Yes")
