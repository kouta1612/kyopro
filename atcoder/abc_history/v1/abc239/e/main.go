...
func main() {
	...
	for i := 0; i < q; i++ {
		a, b := ni()-1, ni()-1
		fmt.Println(vs[a][b].x)
	}
}

func dfs(v, p int) []vertex {
	merge(v, vertex{v: v, x: x[v]})
	for _, nv := range g[v] {
		if nv == p {
			continue
		}
...
func merge(u int, children ...vertex) {
	vs[u] = append(vs[u], children...)

	sort.Slice(vs[u], func(i, j int) bool {
		return vs[u][i].x > vs[u][j].x
	})
