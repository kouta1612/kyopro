...
func main() {
	...
	ox, oy := (x0+xh)/2, (y0+yh)/2
	op0x, op0y := x0-ox, y0-oy

	var RAD = 2.0 * math.Pi / float64(n)

	op1x, op1y := math.Cos(RAD)*op0x-math.Sin(RAD)*op0y, math.Sin(RAD)*op0x+math.Cos(RAD)*op0y
	x, y := op1x+ox, op1y+oy
