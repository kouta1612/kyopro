...
func main() {
	...
	dp[1][1], dpx[1][1], dpy[1][1], dpxy[1][1] = 1, 1, 1, 1
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			...
			dp[i+1][j+1] += dpx[i][j+1] + dpy[i+1][j] + dpxy[i][j]
			dp[i+1][j+1] %= MOD

			dpx[i+1][j+1] = (dp[i+1][j+1] + dpx[i][j+1]) % MOD
			dpy[i+1][j+1] = (dp[i+1][j+1] + dpy[i+1][j]) % MOD
			dpxy[i+1][j+1] = (dp[i+1][j+1] + dpxy[i][j]) % MOD
		}
	}
