...
func main() {
	...
	for i := 0; i < m; i++ {
		a, b := ni()-1, ni()-1
		x, y := ni2()
		g[a] = append(g[a], es{b, x, y})
		g[b] = append(g[b], es{a, -x, -y})
	}
	...
	for len(q) > 0 {
		...
		for _, e := range g[v] {
			if ps[e.v].x != INF && ps[e.v].y != INF {
				continue
			}
			ps[e.v].x = ps[v].x + e.dx
			ps[e.v].y = ps[v].y + e.dy
			q = append(q, e.v)
		}
	}

	for i := 0; i < n; i++ {
		if ps[i].x != INF && ps[i].y != INF {
			fmt.Printf("%v %v\n", ps[i].x, ps[i].y)
		} else {
