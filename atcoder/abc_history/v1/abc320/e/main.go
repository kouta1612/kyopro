...
func main() {
	...
	n, m := ni2()
	t, w, s := make([]int, m), make([]int, m), make([]int, m)
	for i := 0; i < m; i++ {
		t[i], w[i], s[i] = ni3()
	}
	st, wait := set.New(comparator.IntComparator), set.NewMultiSet(func(a, b person) int {
		if a.t < b.t {
			return -1
		} else if a.t > b.t {
			return 1
	...
	for i := 0; i < m; i++ {
		rets := make([]person, 0)
		for iter := wait.Begin(); iter.IsValid() && iter.Value().t <= t[i]; iter.Next() {
			rets = append(rets, iter.Value())
		}
		for _, v := range rets {
			st.Insert(v.i)
		}
		...
		ans[first.Value()] += w[i]
		wait.Insert(person{first.Value(), t[i] + s[i]})
		st.Erase(first.Value())
	}
