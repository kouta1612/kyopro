...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		u--
		v--
		g[u] = append(g[u], edge{v: v, w: w})
		g[v] = append(g[v], edge{v: u, w: w})
	}
...
// color: なし: -1, 白: 0, 黒: 1
func dfs(g [][]edge, v, p, color int, result *[]int) {
	(*result)[v] = color
	for _, edge := range g[v] {
		...
		if edge.w%2 == 0 {
			dfs(g, edge.v, v, color, result)
		} else {
			dfs(g, edge.v, v, 1-color, result)
		}
	}
}
