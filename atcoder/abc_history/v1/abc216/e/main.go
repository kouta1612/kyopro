...
func main() {
	...
	// 全部のアトラクションの楽しさをx以下にするとき操作回数をk以下にできるか
	// 条件を満たすxの最大値を求める → 二分探索
	ng, ok := -1, 2*int(1e9)+1
	for ng+1 < ok {
		m := (ok + ng) / 2
		if check(a, m, k) {
	...
	ans := 0
	r := k
	// 全部のアトラクションの楽しさをx以下にするまで各アトラクションにのる
	for i := 0; i < n; i++ {
		if a[i] <= x {
			continue
		}
		na := a[i] - x
		r -= na
		// 長さna = {a[i], a[i]-1, a[i]-2, ..., a[i] - na + 1}
		ans += (a[i] + a[i] - na + 1) * na / 2
	}
	ans += x * r
