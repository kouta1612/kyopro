...
func main() {
	...
	n, k := ni(), ni()
	ps := map[byte]int{'r': ni(), 's': ni(), 'p': ni()}
	t := ns()

	pair := map[byte]byte{'r': 'p', 's': 'r', 'p': 's'}
	other := map[[2]byte]byte{{'r', 's'}: 'p', {'s', 'p'}: 'r', {'p', 'r'}: 's', {'r', 'r'}: 's', {'s', 's'}: 'r', {'p', 'p'}: 's'}
	plays := make([]byte, 0, n)
	result := 0
	for i := 0; i < len(t); i++ {
		if i+1 <= k {
			result += ps[pair[t[i]]]
			plays = append(plays, pair[t[i]])
		} else {
			if pair[t[i]] == plays[len(plays)-k] {
				if i+k < n {
					plays = append(plays, other[[2]byte{pair[t[i+k]], t[i-k]}])
				} else {
					plays = append(plays, other[[2]byte{pair[t[i-k]], pair[t[i-k]]}])
				}
			} else {
				result += ps[pair[t[i]]]
				plays = append(plays, pair[t[i]])
