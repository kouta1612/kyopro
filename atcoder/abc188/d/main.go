...
func main() {
	...
	es := treemap.New[int, int](comparator.IntComparator)
	for i := 0; i < n; i++ {
		a, b, c := ni3()
		if es.Contains(a) {
			es.Insert(a, es.Find(a).Value()+c)
		} else {
			es.Insert(a, c)
		}
		if es.Contains(b + 1) {
			es.Insert(b+1, es.Find(b+1).Value()-c)
		} else {
			es.Insert(b+1, -c)
		}
	}
	...
	ans, s, pre := 0, 0, 0
	for iter := es.Begin(); iter.IsValid(); iter.Next() {
		ans += min(s, C) * (iter.Key() - pre)
		pre = iter.Key()
		s += iter.Value()
	}
