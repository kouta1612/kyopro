...
func main() {
	...
	factorizes := make([]map[int]int, n)
	for i := 0; i < n; i++ {
		factorizes[i] = factorize(a[i])
	}

	maxfactors := make(map[int]int)
	for _, factorize := range factorizes {
		for k, v := range factorize {
			maxfactors[k] = max(maxfactors[k], v)
		}
	}
	...
	result := mint(0)
	for i := 0; i < n; i++ {
		result = result.add(int(lcm.div(a[i])))
	}
