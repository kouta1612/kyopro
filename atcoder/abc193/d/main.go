...
func main() {
	...
	fmt.Println(float64(cnt) / float64((9*k-8)*(9*k-9)))
}

func score(s string, n int) int {
	cs := make([]int, 10)
	for i := 0; i < 4; i++ {
		cs[int(s[i]-'0')]++
	}
...
func newGraph(m int, weighted, directed bool) *graph {
	g := &graph{make(map[int][]_edge), weighted, directed}
...
func (g *graph) addEdge(u, v, c int) {
	g.datas[u] = append(g.datas[u], _edge{v, c})
	if !g.directed {
		g.datas[v] = append(g.datas[v], _edge{u, c})
	}
}

// priorityqueue用にコストの昇順にソートするcomparatorを作成
func (g *graph) dijkstraComparator(a, b _edge) int {
	if a.c == b.c {
		return 0
	} else if a.c < b.c {
		return -1
