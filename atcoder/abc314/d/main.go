...
func main() {
	...
	s := strings.Split(ns(), "")
	q := ni()
	t, x, c := make([]int, q), make([]int, q), make([]string, q)
	last := -1
	for qi := 0; qi < q; qi++ {
		t[qi], x[qi], c[qi] = ni(), ni()-1, ns()
		if t[qi] == 2 || t[qi] == 3 {
			last = qi
		}
	}
	...
	for qi := 0; qi < q; qi++ {
		if qi == last {
			if t[last] == 2 {
				s = strings.Split(strings.ToLower(strings.Join(s, "")), "")
			} else if t[last] == 3 {
				s = strings.Split(strings.ToUpper(strings.Join(s, "")), "")
			}
		} else if t[qi] == 1 {
			s[x[qi]] = string(c[qi])
		}
