...
func main() {
	...
	for qi := 0; qi < q; qi++ {
		...
		if t == 1 {
			...
			v, k := ni()-1, ni()
			members := uf.members[uf.root(v)]
			if len(members) < k {
				fmt.Println(-1)
			} else {
				fmt.Println(members[k-1] + 1)
			}
		}
	}
}
...
func newUnionFind(n int) *unionfind {
	...
	result.par, result.rank, result.siz, result.members = make([]int, n), make([]int, n), make([]int, n), make([][]int, n)
	for i := 0; i < n; i++ {
		result.par[i] = -1
		result.rank[i] = 0
		result.siz[i] = 1
		result.members[i] = []int{i}
	}
	return result
}
...
func (u *unionfind) unite(x, y int) bool {
	...
	u.members[rx] = append(u.members[rx], u.members[ry]...)
	sort.Ints(u.members[rx])
	reverse(u.members[rx])

	if len(u.members[rx]) > 10 {
		u.members[rx] = u.members[rx][:10]
	}
