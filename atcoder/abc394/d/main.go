...
func solve() bool {
	...
	for i := 0; i < n; i++ {
		...
		if stack[len(stack)-1] == ")" || stack[len(stack)-1] == "]" || stack[len(stack)-1] == ">" {
			...
			if stack[len(stack)-2] == ")" || stack[len(stack)-2] == "]" || stack[len(stack)-2] == ">" {
				return false
			}
		}
		if len(stack) >= 2 {
			if stack[len(stack)-1] == ")" && (stack[len(stack)-2] == "<" || stack[len(stack)-2] == "[") {
				return false
			}
			if stack[len(stack)-1] == "]" && (stack[len(stack)-2] == "(" || stack[len(stack)-2] == "<") {
				return false
			...
			if (stack[len(stack)-2] == "(" && stack[len(stack)-1] == ")") || (stack[len(stack)-2] == "[" && stack[len(stack)-1] == "]") || (stack[len(stack)-2] == "<" && stack[len(stack)-1] == ">") {
