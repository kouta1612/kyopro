...
func main() {
	...
	color, cnt := make([]int, n), make([]int, n)
	for i := 0; i < n; i++ {
		color[i] = i
		cnt[i] = 1
	}
...
type unionfind struct {
	...
	par, rank, siz, l, r []int
}

func newUnionFind(n int) *unionfind {
	result := new(unionfind)
	result.par, result.rank, result.siz, result.l, result.r = make([]int, n), make([]int, n), make([]int, n), make([]int, n), make([]int, n)
	for i := 0; i < n; i++ {
		result.par[i] = -1
		result.rank[i] = 0
		result.siz[i] = 1
...
func (u *unionfind) unite(x, y int) bool {
	...
	u.l[rx] = min(u.l[rx], u.l[ry])
	u.r[rx] = max(u.r[rx], u.r[ry])
