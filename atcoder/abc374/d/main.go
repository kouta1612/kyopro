...
func main() {
	...
	for {
		for b := 0; b < 1<<n; b++ {
			sum, now := 0.0, point{0, 0}
			for i := 0; i < n; i++ {
				move := func(sx, sy, gx, gy int) float64 {
					dx, dy := sx-gx, sy-gy
					return math.Sqrt(float64(dx*dx + dy*dy))
				}
				f := func(sx, sy, gx, gy int) float64 {
					res := 0.0
					res += move(now.i, now.j, sx, sy) / float64(s)
					res += move(sx, sy, gx, gy) / float64(t)
				...
				if b>>i&1 == 0 {
					sum += f(ps[ids[i]][0].i, ps[ids[i]][0].j, ps[ids[i]][1].i, ps[ids[i]][1].j)
				} else {
					sum += f(ps[ids[i]][1].i, ps[ids[i]][1].j, ps[ids[i]][0].i, ps[ids[i]][0].j)
				}
			}
			chmin(&ans, sum)
		}
