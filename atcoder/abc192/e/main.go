...
func main() {
	...
	n, m := ni2()
	x, y := ni()-1, ni()-1
	a, b, t, k := make([]int, m), make([]int, m), make([]int, m), make([]int, m)
	g := make([][]road, n)
	for i := 0; i < n; i++ {
		g[i] = make([]road, 0)
	}
	for i := 0; i < m; i++ {
		a[i], b[i], t[i], k[i] = ni()-1, ni()-1, ni(), ni()
		g[a[i]] = append(g[a[i]], road{b[i], t[i], k[i]})
		g[b[i]] = append(g[b[i]], road{a[i], t[i], k[i]})
	}
	...
	pq := priorityqueue.New[now](func(a, b now) int {
		if a.t < b.t {
			return -1
		} else if a.t > b.t {
			return 1
	...
	for pq.Size() > 0 {
		...
		for _, rd := range g[nw.v] {
			if dist[rd.v] > (nw.t+rd.k-1)/rd.k*rd.k+rd.t {
				dist[rd.v] = (nw.t+rd.k-1)/rd.k*rd.k + rd.t
				pq.Push(now{rd.v, dist[rd.v]})
			}
		}
	}
...
type lazySegmentTree struct {
	n          int
	node, lazy []int
	fx         func(a, b int) int // 値配列の結合関数
	fa         func(a, b int) int // 遅延評価による値配列の更新関数
	fm         func(a, b int) int // 遅延配列の結合関数
	fp         func(a, b int) int // 区間の長さに比例した作用素を適用する関数
	ex         int                // 値配列の単位元
	em         int                // 遅延配列の単位元
}
...
func (m *lazySegmentTreeOption) xor(a, b int) int {
	return a ^ b
}
