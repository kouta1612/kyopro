...
var dx []int = []int{2, 1, -1, -2, -2, -1, 1, 2}
var dy []int = []int{1, 2, 2, 1, -1, -2, -2, -1}

func main() {
	defer out.Flush()

	// 全てのマス目(n^2) - 取られるマス目 = 答え

	n, m := ni2()

	mp := make(map[cell]bool)
	for i := 0; i < m; i++ {
		a, b := ni()-1, ni()-1
		mp[cell{x: a, y: b}] = true
		for j := 0; j < 8; j++ {
			na := a + dx[j]
			nb := b + dy[j]
			...
			mp[cell{x: na, y: nb}] = true
		}
	}

	fmt.Println(n*n - len(mp))
}
