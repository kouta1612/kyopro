...
func main() {
	...
	rids, cids := make([]int, h), make([]int, w)
	for i := 0; i < h; i++ {
		rids[i] = i
	}
	for i := 0; i < w; i++ {
	...
	for {
		for {
			...
			if same(tg, g2) {
				chmin(&ans, f(rids)+f(cids))
			}

			if !next_permutation(cids) {
				break
			}
		}
		if !next_permutation(rids) {
			break
		}
	}
