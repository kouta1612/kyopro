...
func main() {
	...
	var rec func(i, j, k int) float64
	rec = func(i, j, k int) float64 {
		if dp[i][j][k] != 0 {
			return dp[i][j][k]
		}
		if i == 0 && j == 0 && k == 0 {
		...
		sum := float64(n)
		if i > 0 {
			sum += rec(i-1, j, k) * float64(i)
		}
		if j > 0 {
			sum += rec(i+1, j-1, k) * float64(j)
		}
		if k > 0 {
			sum += rec(i, j+1, k-1) * float64(k)
		}
		dp[i][j][k] = sum / float64(i+j+k)

		return dp[i][j][k]
	}

	fmt.Println(rec(mp[1], mp[2], mp[3]))
}
