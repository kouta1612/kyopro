...
func main() {
	...
	n, m := ni2()
	x, y, r := make([]float64, n+m), make([]float64, n+m), make([]float64, n)

	rmax := float64(INF)
	for i := 0; i < n; i++ {
		x[i], y[i], r[i] = nf(), nf(), nf()
		rmax = math.Min(rmax, r[i])
	}
	for i := 0; i < m; i++ {
		x[n+i], y[n+i] = nf(), nf()
	}

	var isOk func(k float64) bool
	isOk = func(k float64) bool {
		res := true
		for i := 0; i < n+m; i++ {
			for j := i + 1; j < n+m; j++ {
				var r1, r2 float64
	...
	ok, ng := 0.0, float64(rmax)
	for math.Abs(ok-ng) > 0.00000001 {
		mid := (ok + ng) / 2
		if isOk(mid) {
			ok = mid
		} else {
